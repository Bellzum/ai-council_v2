...
When implementing features, all tests MUST be labeled with pytest markers for SDLC-stage filtering. This enables running different test suites at different stages of development.

### Test Level Classification (pick exactly one)

| Level | Marker | Scope | Speed | Dependencies |
|-------|--------|-------|-------|--------------|
| Unit | `@pytest.mark.unit` | Single function/class | < 100ms | None/mocked |
| Integration (Whitebox) | `@pytest.mark.integration_whitebox` | 2-3 components, internal APIs | < 1s | Internal access |
| Integration (Blackbox) | `@pytest.mark.integration_blackbox` | 2-3 components, public APIs | < 5s | Public APIs |
| System | `@pytest.mark.system` | Full application E2E | < 30s | All dependencies |
| Acceptance | `@pytest.mark.acceptance` | User workflow validation | Variable | Production-like |
| Validation | `@pytest.mark.validation` | Production env verification | Variable | Real production |

### Test Type Classification (pick at least one)

| Type | Marker | Focus |
|------|--------|-------|
| Functional | `@pytest.mark.functional` | Requirements work correctly |
| Security | `@pytest.mark.security` | Vulnerabilities, auth, injection |
| Performance | `@pytest.mark.performance` | Speed, load, throughput |
| Usability | `@pytest.mark.usability` | UX, accessibility, error messages |

### SDLC Stage → Test Execution

| Stage | What Runs | Command |
|-------|-----------|---------|
| **Feature Testing** | unit + whitebox + acceptance + system | `pytest -m "unit or integration_whitebox or acceptance or system"` |
| **CI Pipeline** | unit + whitebox + blackbox | `pytest -m "unit or integration_whitebox or integration_blackbox"` |
| **Sprint Review** | acceptance + system | `pytest -m "acceptance or system"` |
| **Security Audit** | security | `pytest -m "security"` |
| **Production** | validation | `pytest -m "validation"` |

### Quality Attribute Markers (optional)

| Marker | Description |
|--------|-------------|
| `@pytest.mark.regression` | Prevent bug recurrence |
| `@pytest.mark.falsifiable` | Tests that can definitively fail |
| `@pytest.mark.adversarial` | Red team attack vector tests |
...
# Unit test: isolated, fast (runs during Feature Testing)
@pytest.mark.unit
@pytest.mark.functional
def test_calculate_total():
    """Test Level: unit | Test Type: functional | SDLC: Feature Testing, CI"""
    assert calculate_total([1, 2, 3]) == 6

# Whitebox integration test: component interaction (runs during Feature Testing)
@pytest.mark.integration_whitebox
@pytest.mark.functional
def test_user_repository_save():
    """Test Level: integration_whitebox | Test Type: functional | SDLC: Feature Testing, CI"""
    user = User(name="Alice")
    repo.save(user)
    assert repo.find_by_name("Alice") == user

# Blackbox integration test: public API only (runs during CI Pipeline)
@pytest.mark.integration_blackbox
@pytest.mark.functional
def test_api_create_user():
    """Test Level: integration_blackbox | Test Type: functional | SDLC: CI Pipeline"""
    response = client.post("/users", json={"name": "Alice"})
    assert response.status_code == 201

# Acceptance test: user workflow (runs during Sprint Review)
@pytest.mark.acceptance
@pytest.mark.functional
def test_user_can_complete_checkout():
    """Test Level: acceptance | Test Type: functional | SDLC: Sprint Review"""
...
### Common Scenarios → Classification

| Scenario | Level Marker | Type Marker | SDLC Stage |
|----------|--------------|-------------|------------|
| New function with business logic | `unit` | `functional` | Feature Testing |
| Testing internal component interactions | `integration_whitebox` | `functional` | Feature Testing |
| Testing public API contracts | `integration_blackbox` | `functional` | CI Pipeline |
| Authentication endpoint security | `integration_blackbox` | `security` | CI Pipeline, Security Audit |
| Complete user workflow validation | `acceptance` | `functional` | Sprint Review |
| End-to-end system test | `system` | `functional` | Sprint Review |
| Load testing API performance | `integration_blackbox` | `performance` | CI Pipeline |
| Production smoke tests | `validation` | `functional` | Production |

### Test Classification Checklist

When writing tests, verify:
- [ ] Test has exactly ONE level marker (unit, integration_whitebox, integration_blackbox, system, acceptance, validation)
- [ ] Test has at least ONE type marker (functional, security, performance, usability)
- [ ] Classification matches test behavior (unit tests don't touch real DB)
- [ ] Whitebox tests use internal knowledge, blackbox tests use only public APIs
- [ ] Acceptance tests map to acceptance criteria from work items
- [ ] Tests can be run selectively via SDLC stage commands
