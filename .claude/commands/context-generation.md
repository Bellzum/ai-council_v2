...
Generate hierarchical CLAUDE.md and README.md documentation structure with directed context loading for ai-council.
...
def generate_readme_content(dir_info):
    ...
    dir_name = Path(dir_info['relative_path']).name if dir_info['relative_path'] != '.' else 'ai-council'
...
for dir_info in directories_to_document:
    ...
    if readme_path.exists():
        print(f"  ⏭ {dir_info['relative_path']}/README.md (exists, skipping)")
        continue
    ...
    if content and content.strip():
        with open(readme_path, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"  ✓ {dir_info['relative_path']}/README.md")
    else:
        print(f"  ⚠ {dir_info['relative_path']}/README.md (skipped - empty)")
```
...
def generate_claude_md_content(dir_info, all_dirs):
    ...
    dir_name = Path(relative_path).name if relative_path != '.' else 'ai-council'
    ...
    if claude_path.exists():
        ...
        try:
            ...
            if match:
                ...
                print(f"  🔄 {dir_info['relative_path']}/CLAUDE.md (merged)")
            else:
                print(f"  ⚠ {dir_info['relative_path']}/CLAUDE.md (merge failed)")
        except Exception as e:
            print(f"  ✗ {dir_info['relative_path']}/CLAUDE.md (error: {e})")
    else:
        # Create new file
        if new_content and new_content.strip():
            with open(claude_path, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print(f"  ✓ {dir_info['relative_path']}/CLAUDE.md")
        else:
            print(f"  ⚠ {dir_info['relative_path']}/CLAUDE.md (skipped - empty)")
```
...
for dir_info in directories_to_document:
    ...
    if not claude_path.exists():
        issues.append(f"❌ {relative}/CLAUDE.md: File not created")
        continue
    ...
    try:
        ...
        if not content.strip():
            issues.append(f"❌ {relative}/CLAUDE.md: Empty file")
            continue

        # Check front matter
        if not content.startswith('---'):
            warnings.append(f"⚠️  {relative}/CLAUDE.md: Missing front matter")
            continue

        # Parse YAML
        match = re.match(r'^---\n(.*?)\n---\n', content, re.DOTALL)
        if not match:
            issues.append(f"❌ {relative}/CLAUDE.md: Malformed front matter")
            continue

        # All checks passed
        passed += 1
        print(f"  ✓ {relative}/CLAUDE.md")

    except Exception as e:
        issues.append(f"❌ {relative}/CLAUDE.md: {e}")

print(f"\n{'='*60}")
print(f"Verification Summary")
print(f"{'='*60}")
print(f"  ✅ Passed: {passed}/{len(directories_to_document)}")
...
### Step 5: Generate Context Index

Generate a centralized context index file for O(1) lookups during context loading:
...
def parse_front_matter(claude_path):
...
def extract_root_summary():
    ...
    front_matter = parse_front_matter(root_claude)
    purpose = "Project root"
    if front_matter:
        purpose = front_matter.get('purpose', purpose)

    keywords = front_matter.get('keywords', ['project', 'overview']) if front_matter else ['project', 'overview']

    return {
        "path": "CLAUDE.md",
        "summary": purpose[:200] if isinstance(purpose, str) else str(purpose)[:200],
...
for claude_file in root.rglob("CLAUDE.md"):
    # Skip .git and other hidden directories
    if '.git' in claude_file.parts:
        continue

    # Skip ignored directories
    if any(skip in claude_file.parts for skip in skip_dirs):
        continue

    # Skip root CLAUDE.md (handled in 'root' section)
    if claude_file.parent == root:
        continue

    front_matter = parse_front_matter(claude_file)
    if not front_matter:
        skipped_count += 1
        continue

    try:
        relative_path = str(claude_file.relative_to(root))
    except ValueError:
        skipped_count += 1
        continue
    ...
    readme_path = claude_file.parent / 'README.md'
    if readme_path.exists():
        context_entry["readme"] = str(readme_path.relative_to(root))
...
# Save index file
index_path = root / ".claude" / "context-index.yaml"
...
### Step 6: Enhance CLAUDE.md Content (Optional)
...
if src_claude.exists():
    ...
    if enhancements.strip() != "## Modules":
        ...
        with open(src_claude, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"✓ Enhanced src/CLAUDE.md with module descriptions")
```
...
- [ ] Context index generated at `.claude/context-index.yaml`
- [ ] Keyword index enables O(1) context lookups
...
├── CLAUDE.md                    # Root context (with children directives)
├── .claude/
│   └── context-index.yaml      # Centralized context index (O(1) lookups)
├── src/
