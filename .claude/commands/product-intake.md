...
1. **Work tracking configured** in azure-devops
...
# Optional: Non-interactive mode - set variables instead of prompts
# This enables automated/batch product intake without user interaction
#
# To use non-interactive mode, set these variables before running the workflow:
#   product_title = "Your Product Title"
#   product_description = "Detailed description of the product idea"
#   product_business_value = "Business value explanation (optional)"
#
# If these variables are set, the workflow will skip interactive prompts
# in Phase 1 and use these values instead.
#
# Validation constraints:
#   - title: 1-200 characters (required)
#   - description: 1-10,000 characters (required)
#   - business_value: optional, any length
#
# Example for automated intake:
#   product_title = "User Authentication System"
#   product_description = "Implement OAuth2-based authentication with MFA support for enterprise customers"
#   product_business_value = "Required for enterprise tier (high revenue impact)"
#
# Leave unset (or set to empty string "") for interactive mode.
product_title = ""
product_description = ""
product_business_value = ""
```
...
# Check for non-interactive mode (variables set in initialization)
use_noninteractive = bool(product_title and product_description)

if use_noninteractive:
    print(":rocket: Non-interactive mode detected (using predefined variables)")
    ...
    if not title:
        print(":x: ERROR: product_title cannot be empty")
        sys.exit(1)
    if len(title) > 200:
        print(f":x: ERROR: product_title too long ({len(title)} chars, max 200)")
        sys.exit(1)

    if not description:
        print(":x: ERROR: product_description cannot be empty")
        sys.exit(1)
    if len(description) > 10000:
        print(f":x: ERROR: product_description too long ({len(description)} chars, max 10000)")
        sys.exit(1)
    ...
    # Interactive mode
    print(":keyboard: Interactive mode (prompting for input)")
...
if action in ['c', 'e']:
    ...
    size_to_type = {
        'Small': 'Task',      # 1-2 days → Task
        'Medium': 'Feature',  # 3-5 days → Feature
        'Large': 'Epic',      # 1-2 weeks → Epic (needs Features)
        'Epic': 'Epic'        # Multiple weeks → Epic (needs breakdown)
    }
...
- :white_check_mark: Work item created in azure-devops
