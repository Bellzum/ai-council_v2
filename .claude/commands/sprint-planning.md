...
# This enables automated sprint planning without user interaction
#
# To use non-interactive mode, set these variables before running the workflow:
#   prioritize_epic_ids = [1001, 1002]  # Prioritize specific EPIC IDs (moved to front of queue)
#   exclude_epic_ids = [1005, 1006]  # Exclude specific EPIC IDs from sprint planning
#
# If these variables are set, the workflow will use these values to filter/prioritize EPICs.
#
# Validation constraints:
#   - prioritize_epic_ids: list of EPIC IDs (optional, default: [] = no prioritization)
#   - exclude_epic_ids: list of EPIC IDs (optional, default: [] = no exclusions)
#
# Example for automated planning:
#   prioritize_epic_ids = [1001, 1002]
#   exclude_epic_ids = [1005, 1006]
#
# Leave unset (or set to empty list) for interactive mode.
# Check if variables are defined; if not, use defaults for interactive mode
try:
    prioritize_epic_ids
except NameError:
    prioritize_epic_ids = []  # No prioritization

try:
    exclude_epic_ids
except NameError:
    exclude_epic_ids = []  # No exclusions
```
...
try:
    ...
    sprint_path = f"\\https://dev.azure.com/keychainio/AI%20Council\\Iteration"
    ...
    cmd = [
        ...
        '--project', 'https://dev.azure.com/keychainio/AI%20Council',
        '--output', 'json'
    ]
    ...
    if "already exists" in e.stderr or "duplicate" in e.stderr.lower():
        ...
        list_cmd = ['az', 'boards', 'iteration', 'project', 'list', '--project', 'https://dev.azure.com/keychainio/AI%20Council', '--output', 'json']
...
try:
    ...
    project = "https://dev.azure.com/keychainio/AI%20Council"
    team = "https://dev.azure.com/keychainio/AI%20Council Team"
...
try:
    ...
    if response.status_code == 200:
        ...
        if sprint_active:
            ...
            print(f"   {org_url}/https://dev.azure.com/keychainio/AI%20Council/_sprints/directory")
        else:
...
"""
    ...
    try:
        ...
        result = adapter.create_work_item(
            ...
            fields={
                'System.IterationPath': f'https://dev.azure.com/keychainio/AI%20Council\\{sprint_number}',
...
for item_id in verified_items:
    try:
        ...
        expected_path = f'https://dev.azure.com/keychainio/AI%20Council\\{sprint_number}'
