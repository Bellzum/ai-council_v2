    ...
    def get_current_user(self) -> Dict[str, str]:
        ...
        Returns:
            Dict with keys:
                ...
                - email: User's email (may be same as display_name for some orgs)
        ...
        try:
            ...
            if not user_id:
                raise AuthenticationError(
                    "Could not determine authenticated user. "
                    "Verify your Azure DevOps PAT token is valid."
                )

            # providerDisplayName is usually the email or username
            return {
                "id": user_id,
                "display_name": display_name,
                "email": display_name  # In Azure DevOps, this is typically the email
            }
        except Exception as e:
            if isinstance(e, AuthenticationError):
                raise
